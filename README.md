...
let token = decode::<Claims, Header>(&token, &DecodingKey::from_secret("secret".as_ref()), &Validation::default())?;
```
...
```rust
// `token` is a struct with 2 fields: `header` and `claims` where `claims` is your own struct.
let token = decode::<Claims, Header>(&token, &DecodingKey::from_rsa_components(jwk["n"], jwk["e"]), &Validation::new(Algorithm::RS256))?;
```
