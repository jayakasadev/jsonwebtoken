...
use jsonwebtoken::{decode, decode_header, DecodingKey, GetHeader, Header, Validation};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let Some(kid) = header.get_header().kid else {
    ...
    let validation = {
        let mut validation = Validation::new(header.get_header().alg);
    ...
    let decoded_token =
        decode::<Header, HashMap<String, serde_json::Value>>(TOKEN, &decoding_key, &validation)?;
