...
use jsonwebtoken::{
    ...
    crypto::{sign, verify},
    decode, decode_header, encode, jwt_signer_factory, jwt_verifier_factory,
};
...
fn sign_hs256() {
    let provider =
        jwt_signer_factory(&Algorithm::HS256, &EncodingKey::from_secret(b"secret")).unwrap();
    let mut result = String::new();
    sign(&provider, b"hello world", &mut result).unwrap();
...
fn encode_with_custom_header() {
    ...
    let signing_provider =
        jwt_signer_factory(&header.alg, &EncodingKey::from_secret(b"secret")).unwrap();
    ...
    let decoding_provider =
        jwt_verifier_factory(&header.alg, &DecodingKey::from_secret(b"secret")).unwrap();
...
fn decode_token() {
    ...
    let decoding_provider =
        jwt_verifier_factory(&header.alg, &DecodingKey::from_secret(b"secret")).unwrap();
    let claims = decode::<Claims, Header>(
        &decoding_provider,
        &header.alg,
        token,
        &Validation::new(Algorithm::HS256),
...
fn encode_wrong_alg_family() {
    let header = Header::default();
    let signing_provider =
        jwt_signer_factory(&header.alg, &EncodingKey::from_rsa_der(b"secret")).unwrap_err();
    assert_eq!(signing_provider.into_kind(), ErrorKind::InvalidAlgorithm);
}
...
fn decode_token_with_bytes_secret() {
    ...
    let header: Header = decode_header(token).unwrap();
    let decoding_provider =
        jwt_verifier_factory(&header.alg, &DecodingKey::from_secret(b"\x01\x02\x03")).unwrap();
    let claims = decode::<Claims, Header>(
        &decoding_provider,
        &header.alg,
        token,
...
fn dangerous_insecure_decode_valid_token() {
    ...
    let header: Header = decode_header(token).unwrap();
    let decoding_provider =
        jwt_verifier_factory(&header.alg, &DecodingKey::from_secret(&[])).unwrap();
    let claims = decode::<Claims, Header>(&decoding_provider, &header.alg, token, &validation);
    claims.unwrap();
}
...
fn verify_hs256_rfc7517_appendix_a1() {
    ...
    let c = decode::<C, Header>(&decoding_provider, &header.alg, token, &validation).unwrap();
