use jsonwebtoken::{Algorithm, Header, TokenData, dangerous::insecure_decode};
...
fn dangerous_insecure_decode_valid_jwt() {
    ...
    let TokenData { header, claims } = insecure_decode::<Claims, Header>(token).unwrap();
...
fn dangerous_insecure_decode_invalid_header() {
    ...
    insecure_decode::<Claims, Header>(token).unwrap_err();
}
